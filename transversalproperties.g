...
IdempotentGeneratedProperty := function(G,k)
#
# Suppose  G  is a permutation group on  [1..n],  where
# n:=LargestMovedPoint(G),  and  k  is an integer with  1 < k < n.
# 
# Then this function returns `true' if  G  satisfies
# the  k-id  property, and `false' if not.
#
# The group  G  satisfies the *k-id* or *k-idempotent generated*
# property if for every transformation  f : [1..n] --> [1..n]
# of rank  k,  the semigroup  <G,f> \ G  is generated by its idempotents.
#
local n,setreps,partitionreps,set,partition,pi_2;
if not (IsPermGroup(G) and IsInt(k)) then
   Error("usage: IdempotentGeneratedProperty( <PermGrp>, <Int> )");
fi;
n:=LargestMovedPoint(G);
if k<=1 or k>=n then
...
if k=3 and Transitivity(G)>=2 then
   #
   # Try the fixed-pair routing graphs approach.
   #
   pi_2:=[[1],[2],[3..n]];
   for set in setreps do
      if not IsConnectedGraph(OrderedLiftOfHoughtonGraph(G,pi_2,set)) then
...
Info(TRANSVERSALPROPERTIES_info,2,
   "IdempotentGeneratedProperty: no. of k-partition orbits = ",
...
od;
Info(TRANSVERSALPROPERTIES_info,2,
   "IdempotentGeneratedProperty: k-RCP holds for all orbit-pairs");
for partition in partitionreps do
   for set in setreps do
...
IdempotentGenerationProperty:=IdempotentGeneratedProperty;
